...
if stage == 3 or stage == 0:
    ...
    def Add_Noise(row):
        ...
        data = noise(data)
        sf.write(output_path + '/' + row['file'].split('/')[-1][:-4] + '_noise.wav', data, sample_rate, subtype='PCM_24')
        row['file'] = output_path + '/' + row['file'].split('/')[-1][:-4] + '_noise.wav'
        return row
    
    def Add_Pitch(row):
        data, sample_rate = librosa.load(row['file'], sr=16000)
        data = pitch(data, sample_rate)
        sf.write(output_path + '/' + row['file'].split('/')[-1][:-4] + '_pitch.wav', data, sample_rate, subtype='PCM_24')
        row['file'] = output_path + '/' + row['file'].split('/')[-1][:-4] + '_pitch.wav'
        return row
    
    def Add_Stretch(row):
        data, sample_rate = librosa.load(row['file'], sr=16000)
        data = stretch(data)
        sf.write(output_path + '/' + row['file'].split('/')[-1][:-4] + '_stretch.wav', data, sample_rate, subtype='PCM_24')
        row['file'] = output_path + '/' + row['file'].split('/')[-1][:-4] + '_stretch.wav'
        return row
    
    def Add_Shift(row):
        data, sample_rate = librosa.load(row['file'], sr=16000)
        data = shift(data)
        sf.write(output_path + '/' + row['file'].split('/')[-1][:-4] + '_shift.wav', data, sample_rate, subtype='PCM_24')
        row['file'] = output_path + '/' + row['file'].split('/')[-1][:-4] + '_shift.wav'
        return row
    
    def Add_Cutoffs(row):
        data, sample_rate = librosa.load(row['file'], sr=16000)
        data = cutoffs(data)
        sf.write(output_path + '/' + row['file'].split('/')[-1][:-4] + '_cutoffs.wav', data, sample_rate, subtype='PCM_24')
        row['file'] = output_path + '/' + row['file'].split('/')[-1][:-4] + '_cutoffs.wav'
        return row
    
    tqdm.pandas()
    df2 = df.progress_apply(Add_Noise, axis=1)
    df3 = df.progress_apply(Add_Pitch, axis=1)
    df4 = df.progress_apply(Add_Stretch, axis=1)
    df5 = df.progress_apply(Add_Shift, axis=1)
    df6 = df.progress_apply(Add_Cutoffs, axis=1)
    df7 = df.progress_apply(Add_Cutoffs, axis=1).progress_apply(Add_Noise, axis=1)
    df8 = df.progress_apply(Add_Noise, axis=1).progress_apply(Add_Pitch, axis=1).progress_apply(Add_Stretch, axis=1)
    df9 = df.progress_apply(Add_Noise, axis=1).progress_apply(Add_Pitch, axis=1).progress_apply(Add_Shift, axis=1)
    df10 = df.progress_apply(Add_Cutoffs, axis=1).progress_apply(Add_Noise, axis=1).progress_apply(Add_Pitch, axis=1)
    df = pd.concat([df, df2, df3, df4, df5, df6, df7, df8, df9, df10], axis=0, ignore_index=True)
...
if stage == 4 or stage == 0:
    ...
    tqdm.pandas()
    df = df.progress_apply(Preprocess_s3, axis=1)
